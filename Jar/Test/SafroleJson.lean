...
/-- Run a single safrole test from a JSON file. Returns true if passed. -/
def runJsonTest (path : System.FilePath) : IO Bool := do
  ...
  let expectedResult ← IO.ofExcept (@fromJson? SafroleResult _ (← IO.ofExcept (json.getObjVal? "output")))
  let expectedPost ← IO.ofExcept (@fromJson? FlatSafroleState _ (← IO.ofExcept (json.getObjVal? "post_state")))
