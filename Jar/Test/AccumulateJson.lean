...
/-- Run a single accumulate test from a JSON file. Returns true if passed. -/
def runJsonTest (path : System.FilePath) : IO Bool := do
  ...
  let outputJson ← IO.ofExcept (json.getObjVal? "output")
  let expectedHash ← IO.ofExcept (do
    let okVal ← outputJson.getObjVal? "ok"
    @fromJson? Hash _ okVal)

  let post ← IO.ofExcept (parseGreyState (← IO.ofExcept (json.getObjVal? "post_state")))
